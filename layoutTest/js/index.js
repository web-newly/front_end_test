...
// 商品价格的动态计算
var pricePNode = document.querySelector('.wrapper #content .contentMain .center .right .rightTop .shopDetail .priceArea .price p');
var pNodePrice = parseInt(pricePNode.innerText);
// 给每个dl绑定点击事件
for(let i=0;i<preferencesDlNode.length;i++){
    for(let j=1;j<preferencesDlNode[i].children.length;j++){
        ...
        preferencesDlNode[i].children[j].dataset.price = `${(i*100 + j*10)}`;
    }
    preferencesDlNode[i].onclick = function(event){
        ...
        if(event.target.dataset.isClick){
            if(flagIndexdd[i] != ddIndex){
                ...
                if(flagIndexdd[i] == 0){
                    pricePNode.innerText = pNodePrice + price;
                }else{
                    pricePNode.innerText = pNodePrice - preferencesDlNode[i].children[flagIndexdd[i]].dataset.price+price;
                }
                preferencesDlNode[i].children[flagIndexdd[i]].style.color = '#666';
                flagIndexdd[i] = ddIndex;
                pNodePrice = parseInt(pricePNode.innerText);
            }
        }
    }
}
...
for(let i=0;i<markNode.length;i++){
    markNode[i].children[1].onclick = function(){
        ...
        preferencesDlNode[i].children[flagIndexdd[i]].style.color = '#666';
        pricePNode.innerText = pNodePrice - preferencesDlNode[i].children[flagIndexdd[i]].dataset.price;
        pNodePrice = parseInt(pricePNode.innerText);
        flagIndexdd[i] = 0;
    }
}
